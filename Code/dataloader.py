...
def read_text_iterator(path):
    data_dict=pickle.load(open(path, 'rb'), encoding='latin1')
    for item in zip(data_dict['text_tensors'],data_dict['audio_tensors'],data_dict['video_tensors'],data_dict['label']):
       if len(item[0])>0:
        yield item
...
class Dataset(IterableDataset):
    ...
    def __next__(self):
        ...
        image_tensor=torch.empty(len(item[0]),item[0][0].shape[0])
        ...
        umask=torch.LongTensor([1]*len(item[3]))
        ...
        return text_tensor,aucs_tensor,vid_tensor,label,umask
...
def load_dataset(root_dir):
    ...
    return Dataset(num_lines,src_iterator)
