...
def train_or_eval_model(model, loss_function, dataloader, epoch, optimizer=None, train=False):
    ...
    for i, data in enumerate(dataloader):
        ...
        if train:
            optimizer.zero_grad() 
        textf, audio, video ,label,umask = data[0].cuda(),data[1].cuda(),data[2].cuda(),data[3].cuda(),data[4].cuda()
       ...
       # **************************************************#
        ...
        print("shape i am  UUUU",torch.cat((textf, acouf, visuf), dim=-1).shape) 
        ...
        log_prob, alpha, alpha_f, alpha_b = model(mf((textf, acouf, visuf), dim=-1), umask)
